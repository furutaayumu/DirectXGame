...
void Effect::Initialize(KamataEngine::Model* model, KamataEngine::Vector3 pos, KamataEngine::Vector4 color) {
	...
	std::uniform_real_distribution<float> RandomSize(0.0f, 2.0f);
	std::uniform_real_distribution<float> RandomRotation(-1.0f, 2.0f);
	std::uniform_real_distribution<float> RandomColor(0.0f, 200.0f);
	// NULLポインタチェック
	assert(model);
	model_ = model;
